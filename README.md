...
- `Space`: wait a turn
- Hover your own tile or read the wait button to see whether holding position is safe this turn, plus the incoming damage range if it is not
- Opening the cache chest now lights an **alarm relay** and starts reinforcement waves until you cut the relay terminal
- `R`: restart the room
...
## Current status
- current project: CyberPG v0.1.26 one-room prototype with a cyborg kittygirl hero, readable enemy badges/silhouettes, stronger chest/sigil payoff, explicit Volt Fang drop/pickup flow, combat-preview landing threat warnings, first-step route danger warnings plus truthful clean/threatened click-again route-commit wording, attack-range / kill-odds inspect text, incoming punish damage ranges on route previews and the wait control, on-board enemy intent badges with damage callouts, explicit seer/guard counterplay hints in the enemy intent panel, a gold pulse marker for the live room objective, click-again route commitment, last-seen enemy ghost memory, manual heal flask inventory/use, a heavier heal-flask payoff that can actually swing the first room, truthful hold-position danger readouts on the hero tile and wait button, hunter rush telegraphs plus a real two-tile rush carve, richer in-room environment dressing with vents/cables/terminals/crates/coolant, a new chest-alarm relay phase with countdown reinforcements and a visible shutdown objective, better combat outcome readability, a cleaner public-demo shell, and mouse/touch route preview controls.
